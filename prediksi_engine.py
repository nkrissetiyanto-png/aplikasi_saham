...
def train_models(intraday):
    ...
    X_seq = X_train.values.reshape((X_train.shape[0], 1, X_train.shape[1]))
    model = Sequential([
    ...
    model.fit(X_seq, y_train, epochs=12, batch_size=32, verbose=0)
    return xgb, model

def predict_next_15m(xgb, model, intraday):
    last = smart_money_features(intraday).tail(1)
    X_last = last[["Price","Volume","Return","VWAP_dev","CVD","VolImb","SM_Score"]].values
    X_seq = X_last.reshape((1,1,7))
    prob_xgb = xgb.predict_proba(X_last)[0][1]
    prob_lstm = model.predict(X_seq)[0][0]
    final_prob = (prob_xgb + prob_lstm) / 2
    return final_prob, last["SM_Score"].values[0]
